...
export const DatenproduktVerwaltung = () => {
  const { 
    ...
    weisePersonDatenproduktRolleZu, zuordnungen, entfernePersonVonDatenproduktRolle, setError 
  } = useData();
  ...
  const handleProduktFormSubmit = async (e) => {
    ...
    const success = editingProdukt 
        ? await aktualisiereDatenprodukt(editingProdukt.id, produktData)
        : await erstelleDatenprodukt(produktData);
    if (success) { setShowProduktForm(false); setEditingProdukt(null); } 
  };
  
  const handleOpenProduktForm = (produkt = null) => { setError(null); setFormError(''); setEditingProdukt(produkt); setShowProduktForm(true); };
  const handleDeleteDatenproduktInitiation = (produkt) => { setError(null); setProduktToDelete(produkt); setShowDeleteModal(true); };
  ...
  const handleAssignRoleSubmit = async (e) => {
    ...
    if (!selectedProduktForAssignment || !assignPersonId || !assignRolleId) { setError("Bitte Person und Rolle auswählen."); return; }
