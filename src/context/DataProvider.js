...
export const DataProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribes = collections.map(({ path, setter }) => {
        ...
        return onSnapshot(q, (snapshot) => {
            ...
            console.error(`Error at ${path}:`, err); setError(`Fehler: ${err.code}.`); setLoading(false);
        });
    });
  ...
  const fuegePersonHinzu = async (personDaten) => {
    try {
      ...
      return docRef.id;
    } catch (e) { console.error(e); setError(`Speicherfehler Person: ${e.code}`); return null; }
  };
  ...
  const aktualisierePerson = async (personId, neueDaten) => {
    try {
      ...
      return true;
    } catch (e) { console.error(e); setError(`Update-Fehler Person: ${e.code}`); return false; }
  };
  ...
  const loeschePerson = async (personId) => {
    try {
        ...
        await batch.commit();
        return true;
    } catch (e) { console.error(e); setError(`Löschfehler Person: ${e.code}`); return false; }
  };
  ...
  const fuegeSkillHinzu = async (skillName, color) => {
    ...
    try {
      ...
      return docRef.id;
    } catch (e) { console.error(e); setError("Fehler beim Hinzufügen des Skills."); return null; }
  };
  ...
  const aktualisiereSkill = async (skillId, skillName, color) => {
    ...
    try {
      ...
      return true;
    } catch (e) { console.error(e); setError("Fehler beim Aktualisieren des Skills."); return false; }
  };

  // KORRIGIERTE Logik: Verhindert Löschen, wenn Skill in Benutzung ist.
  const loescheSkill = async (skillId) => {
      const isSkillInUse = personen.some(p => p.skillIds && p.skillIds.includes(skillId));
      if (isSkillInUse) {
          setError("Dieser Skill wird noch verwendet und kann nicht gelöscht werden.");
      ...
      try {
          ...
          return true;
      } catch (e) { console.error(e); setError(`Löschfehler Skill: ${e.code}`); return false; }
  };
  ...
  // (Restliche CRUD-Funktionen bleiben unverändert)
  const erstelleDatenprodukt = async (produktDaten) => { try { const docRef = await addDoc(collection(db, getCollectionPath('datenprodukte')), { ...produktDaten, erstelltAm: new Date().toISOString(), letzteAenderung: new Date().toISOString() }); return docRef.id; } catch(e) { console.error(e); setError(`Speicherfehler: ${e.code}`); return null; }};
  const aktualisiereDatenprodukt = async (produktId, neueDaten) => { try { await updateDoc(doc(db, getCollectionPath('datenprodukte'), produktId), { ...neueDaten, letzteAenderung: new Date().toISOString() }); return true; } catch(e) { console.error(e); setError(`Update-Fehler: ${e.code}`); return false; }};
  const loescheDatenprodukt = async (produktId) => { try { const batch = writeBatch(db); const q = query(collection(db, getCollectionPath('zuordnungen')), where("datenproduktId", "==", produktId)); const assignments = await getDocs(q); assignments.forEach(doc => batch.delete(doc.ref)); batch.delete(doc(db, getCollectionPath('datenprodukte'), produktId)); await batch.commit(); return true; } catch(e) { console.error(e); setError(`Löschfehler: ${e.code}`); return false; }};
  ...
  const entfernePersonVonDatenproduktRolle = async (zuordnungId) => { try { await deleteDoc(doc(db, getCollectionPath('zuordnungen'), zuordnungId)); return true; } catch(e) { console.error(e); setError(`Löschfehler: ${e.code}`); return false; }};
