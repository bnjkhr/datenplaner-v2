...
import { fetchCalendarEvents } from '../pages/api/calendar';
...
export const DataProvider = ({ children, isReadOnly, user }) => {
  ...
  const aktualisierePerson = preventWriteActions(async (personId, neueDaten) => {
    try {
  ...
  const erstelleDatenprodukt = preventWriteActions(async (produktDaten) => {
    try {
      const docRef = await addDoc(collection(db, getCollectionPath('datenprodukte')), {
        ...produktDaten,
        erstelltAm: new Date().toISOString(),
        letzteAenderung: new Date().toISOString(),
      });
  ...
  const aktualisiereDatenprodukt = preventWriteActions(async (produktId, neueDaten) => {
    try {
      await updateDoc(doc(db, getCollectionPath('datenprodukte'), produktId), {
        ...neueDaten,
        letzteAenderung: new Date().toISOString(),
      });
  ...
  const weisePersonDatenproduktRolleZu = preventWriteActions(async (personId, produktId, rolleId) => {
    ...
    try {
      const docRef = await addDoc(collection(db, getCollectionPath('zuordnungen')), {
        personId,
  ...
  const entfernePersonVonDatenproduktRolle = preventWriteActions(async (zuordnungId) => {
    try {
  ...
  const aktualisiereSkill = preventWriteActions(async (skillId, skillName, color) => {
