...
export const DataProvider = ({ children, isReadOnly }) => {
  ...
  const fuegePersonenImBatchHinzu = preventWriteActions(async (personenArray) => { if (!personenArray || personenArray.length === 0) { setError("Keine Personen zum Hinzufügen vorhanden."); return false; } const batch = writeBatch(db); for (const personData of personenArray) { if (!personData.name || !personData.email) { setError(`Fehler in den Daten: Ein Eintrag hat keinen Namen oder keine E-Mail.`); return false; } const newPersonRef = doc(collection(db, getCollectionPath('personen'))); batch.set(newPersonRef, { ...personData, erstelltAm: new Date().toISOString(), letzteAenderung: new Date().toISOString() }); } try { await batch.commit(); return true; } catch (e) { console.error(e); setError(`Fehler beim Batch-Upload: ${e.code || e.message}`); return false; } });
  ...
  return (
    <DataContext.Provider value={{
      ...
      fuegePersonHinzu, aktualisierePerson, loeschePerson, fuegePersonenImBatchHinzu,
